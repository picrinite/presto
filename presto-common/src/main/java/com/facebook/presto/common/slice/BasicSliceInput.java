 ...
 */
package com.facebook.presto.common.slice;
...
/**
 * This class is a FORK of a final class BasicSliceInput in io.airlift:slice:0.38.
 * The goal is to make some methods be overridden differently, for example,
 * SliceInput::readSlice should create a new Slice with copied data in some use cases.
 */
public class BasicSliceInput
        extends FixedLengthSliceInput
{
    ...
    public static void checkPositionIndex(long index, long size)
    {
        if (index < 0 || index > size) {
            throw new IndexOutOfBoundsException(
                    format("index %d should not be negative and not larger than size %d", index, size));
        }
    }
}
